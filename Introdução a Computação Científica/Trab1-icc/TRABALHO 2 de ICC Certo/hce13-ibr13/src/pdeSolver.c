...
void init_inc(double *pts, int ult_pt_bord_dir, int tam_inc, double delta_x) // Inicializador do vetor de incognitas com o ghost layer.
{
	...
	double n; //distancia no eixo x da borda.
	for(i = 0; i < ult_pt_bord_dir; ++i) // Zera todas as posicoes exceto a borda x de cima.
		pts[i] = 0;
	for(n = delta_x; i < tam_inc; ++i, n += delta_x) // Borda de cima de x tem valores gerados pela eq. de acordo com o slide.
		pts[i] = sin(_2PI * n) * sinh(_2PI);
...
{
	int i, j, l, k, ci1, ci2, ci3, tam_inc; // Iterador de loop; idem; idem; idem; cin são consts. inteiras; tam. do vetor inc.
	...
	double inc[tam_inc], lookup_table[tam_inc], cf1, k2, delta_x, delta_y, inv_delta_x2, inv_delta_y2, dx, dy, r, temp_med_mod, temp_med_res, w, pt_ant, aux_res, cf2;
	
	// "incgs" representa matriz de pts a serem calculados na discretizacao; cfn são consts flt pt calculadas uma vez e usadas varias vezes em loop; n e m sao as distancias nos eixos x e y do ponto a ser calculado; r eh o residuo; tempo medio p/ calc de "inc"; tempo medio p/ calc do residuo; constante de relaxamento do metodo SOR; valor da iteracao passada de um pt, auxiliar no calc d res, constante utilizada no calculo do residuo;
	...
	// O primeiro pt esta depois de um pt inutil, uma borda de x e um ponto inutil => valor inicial de i.
	...
	for(j = 0, i = nx + 2, dx = delta_x, dy = delta_y; i < ci1; ++i, ++j, dx += delta_x) // Loop da iteracao GS.
		{
		if(j == ci2) {	j = 0; i += 2; dx = delta_x; dy += delta_y;	} // Pular bordas e ajeitar n / m.
		
		lookup_table[i] = k2 * sin(_2PI * dx) * sinh(_2PI * dy); // Lookup_table p/ k2 * seno * seno hiperbolico.
		}
	...
	for(l = 0, k = 1, temp_med_mod = 0.0f, temp_med_res = 0.0f; l < ni; ++l, ++k)
	{
		...
		likwid_markerStartRegion("Gauss Seidel");
		
		for(j = 0, i = nx + 2; i < ci1; ++i, ++j) // Loop da iteracao GS.
		{
			if(j == ci2) {	j = 0; i += 2;	} // Pular bordas.
			
			pt_ant = inc[i]; // Salva valor anterior para calc.
			inc[i] = inv_delta_x2 * (inc[i - 1] + inc[i + 1]) + inv_delta_y2 * (inc[i - ci3] + inc[i + ci3]) + lookup_table[i];
		...
		likwid_markerStopRegion("Gauss Seidel");
		...
		for(j = 0, i = nx + 2; i < ci1; ++i, ++j) // Loop da soma dos residuos.
		{
			if(j == ci2){	j = 0; i+= 2;	}
			aux_res = - (lookup_table[i]) - (inv_delta_x2 * (inc[i - 1] + inc[i + 1])) - (inv_delta_y2 * (inc[i - ci3] + inc[i + ci3])) + (inc[i] * cf2) ;
