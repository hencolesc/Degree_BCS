...
scanf("%u", &qt_turbinas); // 1ª linha
scanf("%u", qt_potencias); // 2ª linha
vet_potencias = malloc(*qt_potencias*sizeof(unsigned));
for(i = 0; i < *qt_potencias; ++i)
	scanf("%u", &vet_potencias[i]); // Le as qt_potencias da 3ª linha de arq

scanf("%u", qt_alturas); // Quant. de alturas, 4ª linha
vet_alturas = malloc(*qt_alturas*sizeof(unsigned));

for(i = 0; i < *qt_alturas; ++i)
	scanf("%u", &vet_alturas[i]); // Le as qt_alturas da 5ª linha de arq
...
for(i = 0; i < *qt_potencias; ++i) // Cada ponto (j, i) do grafico se relaciona com o rendimento, potencia e altura
	for(j = 0; j < *qt_alturas; ++j)
		{
        scanf("%f", &grafico[0][j + *qt_alturas *i].p_rendimento);
...
{// Testa combinações de turbinas e potencias na altura dada. Seleciona a melhor baseado na maior eficiencia ponderada
