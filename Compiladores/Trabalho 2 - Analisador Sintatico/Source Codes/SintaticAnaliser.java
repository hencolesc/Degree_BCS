...
public class SintaticAnaliser {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // builds slrTable: a sintatic analysis table
        Stack<Integer> stack = new Stack<>(); // stack of integers from slrTable
        stack.push(0); // push first state, need to go back here in ending process
        ...
        for(;;) { // keeps recognizing until lexic error or accept entry
            ...
            if (str == null) { // str null means need to read next lexic
                str = lexicSymbol(LexicAnaliser.execute());
                // recognize a token from array of chars and trim that to a substring
            }
            
            itemTable = slrTable[stack.peek()][SlrColumnsEnum.findByLexic(str)];
            ...
            if (itemTable < 0) {
                ...
                if (itemTable <= E111.ordinal()) {
                    stack.push(itemTable); // stack new state
                    ...
                    if (itemTable <= R40.ordinal()) {
                        ...
                        for(; i < SlrLinesItensEnum.values()[itemTable].getPop(); i++) {
                            stack.pop(); // pops getPop() times
                        }
                        
                        itemTable = slrTable[stack.peek()]
    ...
    /**
     * Builds slrTable from the slr table produced by teached algorithm.
     *
     * @return the slrTable
     */
    ...
    /**
     * Trim a substring between '<' and ';'
     * @return a substring
     */
    private static String lexicSymbol(String lexicItem) {
        StringBuilder sb = new StringBuilder();
        int i = 1; // lexicItem.charAt[0] = '<', remove that
        
        for (; lexicItem.charAt(i) != ';'; i++) {
            sb.append(lexicItem.charAt(i)); // append a char until finding ';'
        }
